...
class BrowserTool:
    ...
    async def extract_elements(
        ...
        crawl_results = results if isinstance(results, list) else getattr(results, 'results', [results])
        ...
        for result in crawl_results:
            if not result.success:
                ...
                if "playwright install" in error_msg or "Executable doesn't exist" in error_msg:
                    logger.critical("🚨 PLAYWRIGHT BROWSERS MISSING! Run 'playwright install chromium' and 'playwright install-deps chromium' in your python environment.")
